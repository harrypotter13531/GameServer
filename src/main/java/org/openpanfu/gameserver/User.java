	...
	public void handlePacket(PanfuPacket panfuPacket) {
		...
		if (!this.isLoggedIn) {
			if (panfuPacket.getHeader() != Packets.CMD_LOGIN && panfuPacket.getHeader() != Packets.CMD_GET_SALT
					&& panfuPacket.getHeader() != Packets.CMD_INFOMESSAGE) {
				Logger.warning(
						"User tried inputting a forbidden packet for the state he is in! (Not logged in) Attempted packet: "
								+ panfuPacket.getHeader());
				return;
			}
		}
		...
		if (packetHandler == null) {
			Logger.warning("Unhandled packet: " + Packets.headerToName(panfuPacket.getHeader()) + " ("
					+ panfuPacket.getHeader() + ")");
		} else {
	...
	public void sendForReceiver(PanfuPacket PP, String receiver) {
		if (receiver.contains(",")) {
			...
			if (split[0].equals(String.valueOf(PlayerToPlayerCommands.P2P_RECEIVER_ROOM))) {
				List<User> users = getGameServer().getSessionManager().getUsersInRoom(this.getRoomId(), this.inHome,
						this.subRoom);
				for (User user : users) {
					if (user.getUserId() == this.getUserId() && user.getUserId() != Integer.valueOf(split[1]))
						continue;
